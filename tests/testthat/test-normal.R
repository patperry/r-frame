...
test_that("logical", {
    x <- c(TRUE, FALSE, NA)
    expect_equal(as.logical(as.normal(x)), x)
})
...
test_that("hexmode", {
    x <- c(-.Machine$integer.max, -1L, 0L, 1L, .Machine$integer.max, NA_integer_)
    h <- as.hexmode(x)
    expect_equal(as.normal(h), as.normal(as.numeric(x)))
})
...
test_that("record", {
    ...
    expect_equal(as.normal.dataset(x), as.normal(y))
})

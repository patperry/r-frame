...
test_that("logical", {
    x <- c(TRUE, FALSE, NA)
    expect_equal(as.simple(x), x)
})


test_that("raw", {
    x <- as.raw(c(0, 1, 254, 255))
    expect_equal(as.simple(x), as.integer(x))
})
...
test_that("integer", {
    ...
    expect_equal(as.simple(x), x)
    expect_true(is.integer(as.simple(x)))
})
...
test_that("list of heterogeneous", {
    ...
    expect_error(as.simple(l),
                 "cannot convert heterogeneous list to simple")
})
...
test_that("rank 3", {
    ...
    expect_error(as.simple(x),
                 "cannot convert rank-3 objects to simple")
})


test_that("unknown class", {
    x <- structure(list(x = 1), class = "foo")
    expect_error(as.simple(x), "cannot convert objects of class \"foo\" to simple")
})


test_that("invalid mode", {
    expect_error(as.simple(sin), "cannot convert objects of mode \"function\" to simple")
})
